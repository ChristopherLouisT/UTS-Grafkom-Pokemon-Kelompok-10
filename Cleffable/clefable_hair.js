export function generateClefable_Hair(radius, tubeRadius, height, stacks, steps) {
    let vertices = [];
    let faces = [];
    ...
    // --- Shape configuration ---
    const tailLength = 0;  // straight portion before spiral
    const baseStraight = 0;        // proportion of total length that stays straight
    const turns = 1;                // total number of spiral turns
    const taper = 0.2;                // how much smaller the spiral gets at the center
    const verticalLift = 0.2;        // upward lift for organic shape
